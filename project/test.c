...
// Function to find the current address using OpenStreetMap Nominatim API
void find_current_address() {
    FILE *fp;
    char output[100];
    char input[100];
    char lonlat[2][20];
...
void find_nearest_hospitals(char * latitude, char * longitude) {
    ...
    float radius; // Buffer to store response data
    ...
    if (curl) {
        ...
        sprintf(overpass_query, "[out:json];node['amenity'='hospital'](around:%f,%s,%s);out;",radius,  latitude, longitude);
...
int main() {
    ...
    find_current_address();

    // Find nearest hospitals
    find_nearest_hospitals("20.2389807", "-85.8326089");
